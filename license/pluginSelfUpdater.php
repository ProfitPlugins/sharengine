...
class pluginSelfUpdater {
    ...
    public function update_check( $transient ) {
        ...
        if(isset($response->errors))
        {
            ...
            $new_options = get_option(SE_UPDATER_SETTINGS);

            $new_options['api_key'] = '';
            $new_options['email'] = '';
            $new_options['activated'] = 'Deactivated';

            //update_option(SE_UPDATER_SETTINGS, $new_options);

           // var_dump(get_option(SE_UPDATER_SETTINGS));
...
//        }
        ...
        if ( isset( $new_ver ) && isset( $curr_ver ) ) {
            ...
            if( $response !== false && version_compare( $new_ver, $curr_ver, '>' ) ) {
                ...
                if ( $this->plugin_or_theme == 'plugin' ) {
                    ...
                    $transient->response[SE_UPDATE_SLUG] = array (
    ...
    public function no_key_error_notice( $message ){
        ...
        echo sprintf( '<div id="message" class="error"><p>' . __( 'A license key for %s could not be found. Please go to the Sharengine admin and enter your license key. You can reactivate or purchase a license key from your account <a href="%s" target="_blank">dashboard</a>.', $this->text_domain ) . '</p></div>', $plugin_name, $plugin_name, $this->renew_license_url ) ;
