...
//@TODO: refator all of this
add_action('init', 'se_user_avatar_core_set_avatar_constants', 8 );
add_action('se_show_profile', 'se_user_avatar_form');
//add_action('edit_user_profile', 'se_user_avatar_form');
add_action('wp_ajax_se_user_avatar_add_photo', 'se_user_avatar_add_photo');



add_action('user_avatar_iframe_head','se_user_avatar_init');


//add_action('admin_print_styles-user-edit.php', 'se_user_avatar_admin_print_styles');
add_action('admin_print_styles', 'se_user_avatar_admin_print_styles');



function se_user_avatar_admin_print_styles() {
    global $hook_suffix;
    wp_enqueue_script("thickbox");
    wp_enqueue_style("thickbox");
    wp_enqueue_style('user-avatar', SE_URL .'css/user_avatar.css', 'css');
}


/**
 * se_user_avatar_init function.
 * Description: Initializing user avatar style.
 * @access public
 * @return void
 */
function se_user_avatar_init(){
...
/**
 * se_user_avatar_core_set_avatar_constants function.
 * Description: Establishing restraints on sizes of files and dimensions of images.
 * Sets the default constants
 * @access public
 * @return void
 */
function se_user_avatar_core_set_avatar_constants() {

    global $bp;

    if ( !defined( 'USER_AVATAR_UPLOAD_PATH' ) )
        define( 'USER_AVATAR_UPLOAD_PATH', se_user_avatar_core_avatar_upload_path() );

    if ( !defined( 'USER_AVATAR_URL' ) )
        define( 'USER_AVATAR_URL', se_user_avatar_core_avatar_url() );
    ...
    if ( !defined( 'USER_AVATAR_DEFAULT' ) )
        define( 'USER_AVATAR_DEFAULT', SE_PL_USERAVATAR_DIR.'images/mystery-man.jpg' );

    if ( !defined( 'USER_AVATAR_DEFAULT_THUMB' ) )
        define( 'USER_AVATAR_DEFAULT_THUMB', SE_PL_USERAVATAR_DIR.'images/mystery-man-50.jpg' );
 ...
 * @return void
 */
function se_user_avatar_add_photo() {
global $current_user;
...
{
    case 1:
        se_user_avatar_add_photo_step1($uid);
        break;

    case 2:
        se_user_avatar_add_photo_step2($uid);
        break;

    case 3:
        se_user_avatar_add_photo_step3($uid);
        break;
}
...
/**
 * se_user_avatar_add_photo_step1 function.
 ...
 * @return void
 */
function se_user_avatar_add_photo_step1($uid)
{
    ?>
    <p id="step1-image" >
        <?php
        echo se_user_avatar_get_avatar( $uid , 150);
        ?>
    </p>
    <div id="user-avatar-step1">
        <form enctype="multipart/form-data" id="uploadForm" method="POST" action="<?php echo admin_url('admin-ajax.php'); ?>?action=se_user_avatar_add_photo&step=2&uid=<?php echo $uid; ?>" >
...
/**
 * se_user_avatar_add_photo_step2 function.
 ...
 * @return void
 */
function se_user_avatar_add_photo_step2($uid)
{
    ...
    set_transient( 'avatar_file_'.$uid, str_replace("\\","/",$file), 60 * 60 * 5 );
    update_user_meta( $uid,'avatar_file', str_replace("\\","/",$file) );
    ...
    ?>
    <form id="iframe-crop-form" method="POST" action="<?php echo admin_url('admin-ajax.php'); ?>?action=se_user_avatar_add_photo&step=3&uid=<?php echo esc_attr($uid); ?>">
...
/**
 * se_user_avatar_add_photo_step3 function.
 ...
 * @return void
 */
function se_user_avatar_add_photo_step3($uid)
{
    ...
    $original_file =get_user_meta( $uid,'avatar_file',true);
    ...
    se_se_user_avatar_delete_files($uid);
    ...
    ?>
    <script type="text/javascript">
        self.parent.user_avatar_refresh_image('<?php echo se_user_avatar_get_avatar($uid, 150); ?>');
...
/**
 * se_se_user_avatar_delete_files function.
 * Description: Deletes the avatar files based on the user id.
 * @access public
 * @param mixed $uid
 * @return void
 */
function se_se_user_avatar_delete_files($uid)
{
 ...
 * @return <type>
 */
function se_se_user_avatar_fetch_avatar_filter( $avatar, $user, $size, $default, $alt ) {
    global $pagenow;
    ...
    if( se_user_avatar_avatar_exists($id) ):

        $user_avatar = se_user_avatar_fetch_avatar( array( 'item_id' => $id, 'width' => $size, 'height' => $size, 'alt' => $alt ) );
        if($user_avatar)
            return $user_avatar;
        else
            return !empty( $avatar ) ? $avatar : $default;
...
add_filter( 'get_avatar', 'se_se_user_avatar_fetch_avatar_filter', 10, 5 );
 ...
 */
function se_user_avatar_fetch_avatar( $args = '' ) {
    ...
    if( $avatar_img = se_user_avatar_avatar_exists( $item_id ) ):
        ...
        $avatar_url = SE_URL.'user-avatar-pic.php'."?src=".$avatar_src ."&w=".$width."&id=".$item_id."&random=".$file_time;
...
add_action("admin_init", "se_user_avatar_delete");
/**
 * se_user_avatar_delete function.
 *
 * @access public
 * @return void
 */
function se_user_avatar_delete(){
...
function se_user_avatar_form($profile)
{
    ...
    if( $profile->ID == $current_user->ID || current_user_can('edit_user', $current_user->ID) || is_super_admin($current_user->ID) )
    {
        ...
        ?>
        <div <?php if($wp_version < 3.8) echo "style='top:100px'";?> id="user-avatar-display" class="submitbox" >
            <h3 ><?php _e('Picture','user-avatar'); ?></h3>
            <p id="user-avatar-display-image"><?php echo se_user_avatar_get_avatar($profile->ID, 150); ?></p>
            <a id="user-avatar-link" class="button-primary thickbox" href="<?php echo admin_url('admin-ajax.php'); ?>?action=se_user_avatar_add_photo&step=1&uid=<?php echo $profile->ID; ?>&TB_iframe=true&width=720&height=450" title="<?php _e('Upload and Crop an Image to be Displayed','user-avatar'); ?>" ><?php _e('Update Picture','user-avatar'); ?></a>
            ...
            if ( se_user_avatar_avatar_exists($profile->ID) ):?>
...
/*-- HELPER FUNCTIONS --*/
/**
 * se_user_avatar_avatar_exists function.
 *
 * @access public
 * @param mixed $id
 * @return void
 */
function se_user_avatar_avatar_exists($id){
...
/**
 * se_user_avatar_get_avatar function.
 *
 * @access public
 * @param mixed $id
 * @param mixed $width
 * @return void
 */
function se_user_avatar_get_avatar($id,$width) {

    if(! get_option('show_avatars')):

        if( se_user_avatar_avatar_exists($id) ):

            $user_avatar = se_user_avatar_fetch_avatar( array( 'item_id' => $id, 'width' => $width, 'height' => $width, 'alt' => '' ) );
            if($user_avatar):
                return $user_avatar;
            else:
